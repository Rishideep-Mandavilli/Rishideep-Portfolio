...
const TEXT_FONT_SIZE = 140; // Font size for text mask sampling
...
function AutoCamera() {
  ...
  useEffect(() => {
    const resize = () => {
      if (!fitted && camera instanceof THREE.PerspectiveCamera) {
...
function buildGrid(canvasWidth: number, canvasHeight: number) {
  ...
  maskCtx.font = `bold ${TEXT_FONT_SIZE}px "Inter", "Arial", sans-serif`;
  maskCtx.textAlign = "center";
  maskCtx.textBaseline = "middle";

  // Handle multi-line text
  const lines = TEXT.includes("\n") ? TEXT.split("\n") : [TEXT];
  const lineH = TEXT_FONT_SIZE * 1.15;
