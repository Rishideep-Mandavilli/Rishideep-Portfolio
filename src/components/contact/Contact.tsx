    ...
    <section className="relative min-h-screen overflow-hidden px-6 py-24 md:px-10">
      ...
      <div className="mb-10 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div>
          ...
          <h2 className="text-4xl font-semibold text-stone-100 md:text-6xl">Route a signal.</h2>
        </div>
      ...
      <div className="grid gap-3 lg:hidden">
        {DATA.map((node) => (
          <button
            key={node.id}
            onClick={node.action}
            className="flex items-center gap-4 border border-stone-800 bg-black/75 p-4 text-left"
          >
            <span
              className="grid h-10 w-10 place-items-center border border-stone-800 bg-stone-950"
              style={{ color: node.color }}
            >
              {node.icon}
            </span>
            <span className="min-w-0 flex-1">
              <span className="block font-semibold text-stone-100">{node.label}</span>
              <span className="block truncate text-sm text-stone-500">{node.desc}</span>
            </span>
      ...
      >
        ...
        <div className="absolute select-none" style={{ left: sx - SRC_W / 2, top: sy - SRC_H / 2, width: SRC_W, height: SRC_H, zIndex: 15 }}>
          ...
          >
            {/* Header */}
            <div className="h-[3px] w-full bg-stone-500" />
            {/* Body */}
        ...
        <div className="absolute bottom-0 left-0 right-0 h-8 bg-white/70 backdrop-blur-sm border-t border-stone-200/50 flex items-center px-4 gap-4" style={{ zIndex: 10 }}>
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${dragging ? "bg-[#d6ae7c] animate-pulse" : "bg-[#a4d3cc]"}`} />
