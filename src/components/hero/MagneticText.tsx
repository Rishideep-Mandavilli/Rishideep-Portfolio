...
// Field particles — cool grays so the name reads as the only signal
const FIELD = [
  new THREE.Color("#2a3040"),
  new THREE.Color("#222834"),
  new THREE.Color("#333b4d"),
  new THREE.Color("#1a1f29"),
  new THREE.Color("#3d4658"),
];

const SIGNAL = new THREE.Color("#c8f550");
...
function buildGrid(canvasWidth: number, canvasHeight: number) {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      const color = isText
        ? SIGNAL.clone()
        : FIELD[Math.floor(Math.random() * FIELD.length)].clone();
