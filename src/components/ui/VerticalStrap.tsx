...
function VerticalStrap() {
  ...
  return (
    <div
      className="fixed z-[9985] hidden pointer-events-none min-[1500px]:block"
      style={{
        bottom: 24,
        right: 28,
        width: STRAP_W,
        height: `min(${VISIBLE_ITEMS * ITEM_H + 8}px, calc(100svh - 120px))`,
      }}
    >
      <div
        className="relative w-full h-full overflow-hidden rounded-full border border-line-strong bg-panel/85 shadow-[0_0_0_1px_rgba(255,255,255,0.04)_inset] cursor-default pointer-events-auto"
        onMouseEnter={() => setPaused(true)}
        onMouseLeave={() => setPaused(false)}
      >
        {/* Top Fade */}
        <div className="absolute top-0 left-0 right-0 h-12 z-10 bg-panel pointer-events-none" />

        {/* Bottom Fade */}
        <div className="absolute bottom-0 left-0 right-0 h-12 z-10 bg-panel pointer-events-none" />
        ...
        >
          {[...ITEMS, ...ITEMS].map((item, i) => (
            ...
            >
              {item.type === "sep" ? (
                <span className="vertical-text text-[8px] text-dim">
                  {item.label}
                </span>
              ) : item.type === "name" ? (
                <span className="vertical-text text-[11px] font-semibold text-paper">
                  {item.label}
                </span>
              ) : item.type === "status" ? (
                <span className="vertical-text text-[10px] font-semibold text-signal">
                  ● {item.label}
                </span>
              ) : item.type === "tag" ? (
                <span className="vertical-text text-[10px] font-semibold text-dim">
                  {item.label}
                </span>
              ) : (
                <span className="vertical-text text-[10px] text-faint">
                  {item.label}
                </span>
              )}
            </div>
